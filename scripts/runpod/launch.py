...
Docker command: by default the launcher passes docker_args=/start.sh so custom
images based on RunPod base images keep the SSH bootstrap alive. Training is
started later inside tmux after rsync.
...
DEFAULT_DOCKER_ARGS = "/start.sh"
REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
REMOTE_DIR = "/workspace/ppl-synthesis-reward-hacking"
REMOTE_ENV_FILE = f"{REMOTE_DIR}/.runpod_env"
SSH_OPTIONS = [
    "-o",
...
RSYNC_MAX_RETRIES = 3
RSYNC_RETRY_DELAY = 5
SSH_BOOTSTRAP_MAX_RETRIES = 6
SSH_BOOTSTRAP_RETRY_DELAY = 10
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    ...
    parser.add_argument(
        "--docker-args",
        default=DEFAULT_DOCKER_ARGS,
        help=(
            "Docker command/args for the pod. Default keeps RunPod's /start.sh "
            "SSH bootstrap even when a custom image overrides CMD."
        ),
    )
    parser.add_argument(
...
def write_remote_env(
    ...
    """Write forwarded env vars for SSH-started tmux commands."""
    lines = ["# Generated by scripts/runpod/launch.py; do not commit.\n"]
    for key, val in sorted(env.items()):
        if val:
            lines.append(f"export {key}={shlex.quote(val)}\n")
    content = "".join(lines)
    remote_cmd = f"umask 077 && cat > {shlex.quote(REMOTE_ENV_FILE)}"
    subprocess.run(
        [*ssh_cmd(ip, port, identity_file), remote_cmd],
        input=content,
        text=True,
        check=True,
    )
...
def rsync_to_pod(ip: str, port: int, identity_file: str | None = None) -> None:
    ...
    for attempt in range(1, SSH_BOOTSTRAP_MAX_RETRIES + 1):
        result = subprocess.run(bootstrap_cmd)
        if result.returncode == 0:
            break
        if attempt < SSH_BOOTSTRAP_MAX_RETRIES:
            print(
                f"SSH bootstrap attempt {attempt} failed, "
                f"retrying in {SSH_BOOTSTRAP_RETRY_DELAY}s..."
            )
            time.sleep(SSH_BOOTSTRAP_RETRY_DELAY)
    else:
        raise RuntimeError(
            f"SSH bootstrap failed after {SSH_BOOTSTRAP_MAX_RETRIES} attempts"
        )
...
def main() -> None:
    ...
    try:
        ...
        write_remote_env(ip, ssh_port, pod_env, identity_file=args.identity_file)
