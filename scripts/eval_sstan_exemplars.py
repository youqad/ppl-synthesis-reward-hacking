...
def write_markdown_table(results: list[dict[str, Any]], path: Path) -> None:
    ...
    for r in results:
        ...
        lines.append(f"| {r['id']} | {desc} | {tags} | {transpile} | {regex} | {cms} | {expected} |")
    lines.append("")
...
def write_latex_table(results: list[dict[str, Any]], path: Path, *, use_cmdsafestan: bool) -> None:
    ...
    lines.append("\\caption{SStan exemplar evaluation: mitigation pipeline on discovered LH programs "
    ...
    if lh_programs:
        lines.append("\\multicolumn{%d}{l}{\\textit{LH programs}} \\\\" % (
            6 if use_cmdsafestan else 5
        ))
        for r in lh_programs:
            has_heuristic = bool(r["safepymc_tags"])
            desc = _tex_escape(r["description"][:40])
            row = f"{r['id']} & {desc} & {_tex_check(has_heuristic)} & {_tex_check(r['transpile_ok'])}"
            regex_rejected = r["regex_accept"] is False
            row += f" & {_tex_check(regex_rejected)}"
            if use_cmdsafestan:
                cms_rejected = r.get("cmdsafestan_safe") is False
                row += f" & {_tex_check(cms_rejected if r.get('cmdsafestan_safe') is not None else None)}"
            row += " \\\\"
            lines.append(row)

    if honest_programs:
        lines.append("\\midrule")
        lines.append("\\multicolumn{%d}{l}{\\textit{Honest baselines}} \\\\" % (
            6 if use_cmdsafestan else 5
        ))
